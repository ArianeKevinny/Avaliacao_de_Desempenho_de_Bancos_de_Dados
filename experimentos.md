...
| 5 | Atualizar reputação em -10 para usuários que possuem postagens que foram votadas como como Spam| `UPDATE "Users" SET "Reputation" = "Reputation" - 10 WHERE "Id" IN (SELECT DISTINCT p."OwnerUserId" FROM "Posts" p JOIN "Votes" v ON p."Id" = v."PostId" WHERE v."VoteTypeId" = 12 AND p."OwnerUserId" IS NOT NULL);` | consulta otimizada | justifica_otimização | Alteração de registros com subconsulta |
